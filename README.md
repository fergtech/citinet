...
| Feature | Status |
|---|---|
| Hub creation wizard (7-step, generates OS setup script) | ✅ |
...
| Messages — DMs, group conversations, file attachments, full-history search, 1:1 calls & broadcasts (LiveKit, opt-in)* | ✅ |
...
\* Calls/broadcasts require the `citinet-livekit` container — an opt-in step in the creation wizard (2026-09-08), off by default; see [The Hub Stack](#the-hub-stack).
...
by default — a second, unrelated hub can run on the same machine with no collisions. See
[Running Multiple Hubs on One Machine](./docs/hub-setup.md#running-multiple-hubs-on-one-machine).
...
> **Comms (2026-09-08):** `citinet-livekit` is now an opt-in wizard step (off by default —
> an extra service plus newly exposed media ports not every self-hoster wants). With
> `local` (HTTPS bridge) visibility it's fully wired up end to end, including
> `LIVEKIT_PUBLIC_URL` and the Caddy `/rtc*` route. With `tailscale` visibility the
> container and secrets are still provisioned, but `LIVEKIT_PUBLIC_URL` ships blank —
> routing LiveKit's signaling over Tailscale Funnel needs its own `tailscale serve` path
> rule, which isn't auto-configured yet (comms 503s gracefully until that's set up
> manually). A LAN-discovery mDNS advertiser separately exists and runs in production
> (e.g. hub1) but still isn't part of the wizard's output.
...
Hub data is split across four bind-mount paths, each independently set in `~/citinet-hub-<hub-slug>/.env`. All default to subfolders under `~/citinet-hub-<hub-slug>/data/` next to `docker-compose.yml`, but each can point at a different drive — internal, external, USB, NAS.

```env
# ~/citinet-hub-<hub-slug>/.env — all optional, shown with their defaults
...
cp -r ~/citinet-hub-<hub-slug>/data/storage /mnt/new-drive/citinet-files   # e.g. moving just file storage
