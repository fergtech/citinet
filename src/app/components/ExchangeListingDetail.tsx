    ...
    <div className="min-h-screen bg-zinc-950">
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-7">
        <div className="flex items-center gap-3 mb-5">
          <button
            onClick={onBack}
            className="inline-flex items-center gap-1.5 border border-zinc-800 bg-zinc-900 text-slate-300 text-sm font-semibold px-3.5 py-1.5 rounded-full hover:bg-zinc-800 transition-colors"
          >
          ...
          <button
            ...
            className="w-9 h-9 rounded-full flex items-center justify-center text-slate-300 hover:bg-zinc-900 transition-colors"
          >
          ...
          <div className="flex flex-col gap-4.5" style={{ gap: 18 }}>
            <div>
              <div className="flex items-start gap-3">
                <h1 className="flex-1 text-2xl font-bold text-white tracking-tight leading-tight">{listing.title}</h1>
                <span className={`font-mono text-xl font-bold whitespace-nowrap ${listing.price_type === 'fixed' ? 'text-emerald-300' : 'text-white'}`}>{formatPrice(listing)}</span>
              </div>
              <div className="flex items-center gap-2.5 mt-2 text-xs text-slate-400">
                ...
                <span>·</span>
                <span className="uppercase tracking-wide font-semibold text-slate-300">{listing.category}</span>
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
              {vendorLoading ? (
                <div className="flex items-center gap-2 text-sm text-slate-500 py-2"><Loader2 className="w-4 h-4 animate-spin" /> Loading seller…</div>
              ) : (
                <>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-white truncate">{listing.vendor_name ?? 'Unknown seller'}</div>
                    ...
                    <button
                      ...
                      className="text-xs font-semibold text-slate-300 border border-zinc-700 rounded-lg px-2.5 py-1.5 hover:bg-zinc-800 transition-colors shrink-0"
                    >
                      View
                    </button>
                  </div>
            ...
            {moreFromSeller.length > 0 && (
              <div>
                <div className="text-sm font-semibold text-slate-300 mb-2.5">More from {listing.vendor_name?.split(' ')[0] ?? 'this seller'}</div>
        ...
        {/* Sticky action bar */}
        <div className="sticky bottom-0 mt-6 pt-4 pb-4">
          <div className="absolute inset-x-0 bottom-0 top-0 -mx-8 bg-gradient-to-t from-zinc-950 via-zinc-950/95 to-transparent pointer-events-none" />
          ...
          {messageError && <p className="relative text-xs text-red-400 mt-2">{messageError}</p>}
        </div>
      </div>
    </div>
  );
