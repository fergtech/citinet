      ...
      return (
        <ul key={idx} className="list-disc list-inside mb-4 space-y-1">
          {node.content?.map((item: any, itemIdx: number) => (
            <li key={itemIdx} className="text-slate-800 dark:text-zinc-200">
...
export function NoteDetailModal({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      onClick={onClose}
      className="fixed inset-0 z-50 bg-slate-900/40 dark:bg-black/50 flex items-center justify-center p-4"
    >
      <motion.div
      ...
      >
        ...
        <div className="sticky top-0 z-10 bg-white dark:bg-zinc-900 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between p-5">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 flex items-center justify-center text-slate-500 hover:text-slate-700 dark:text-zinc-400 dark:hover:text-zinc-200 transition-colors shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          {/* Body */}
          <div className="text-slate-800 dark:text-zinc-200 space-y-2">
            {note.body_rich ? (
          ...
          <div className="flex gap-2 pt-4 border-t border-slate-100 dark:border-zinc-800">
            <button
              onClick={handleCopy}
              className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 text-sm font-medium text-slate-700 dark:text-zinc-300 transition-colors"
            >
              {copied ? (
                <>
