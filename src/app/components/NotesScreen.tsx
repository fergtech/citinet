...
function NoteEditor({
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      // A hub/web/blog-public note's body must stay plaintext across edits —
      // pass its current public state so updateNote() knows not to re-encrypt it.
      if (latestNote.current.is_public) {
...
export function NotesScreen({ onBack, initialNoteId }: NotesScreenProps) {
  ...
  const handleSetNoteVisibility = async (target: 'private' | 'hub' | 'web' | 'blog') => {
    ...
    setActionsOpen(false);
    setVisError('');
    // A note this device couldn't decrypt is showing placeholder text, not real
    // content — never let that placeholder get published as if it were the note.
    if (target !== 'private' && selected.decryptFailed) {
      setVisError("Can't share this note — this device doesn't have the encryption key it was created with. Open it on the device it was created on first.");
      return;
    }
    try {
      let updated: HubNote;
      if (target === 'blog') {
        ...
        // and for the main body_plain/body_rich fields too (is_public gates those for
        // in-app reads, same as web_body_plain/rich gates the public web snapshot).
        ...
        updated = await hubService.updateNote(hubSlug, selected.id, {
          is_public: true, is_blog_published: true,
          body_plain: selected.body_plain, body_rich: selected.body_rich,
        });
        ...
        updated = await hubService.updateNote(hubSlug, selected.id, {
          is_public: true, is_blog_published: false,
          body_plain: selected.body_plain, body_rich: selected.body_rich,
        });
      ...
      `}>
        {selected ? (
          <>
            ...
            {/* Visibility error — e.g. tried to share a note this device can't decrypt */}
            {visError && (
              <div className="flex items-start gap-2 px-4 py-2 bg-red-50 dark:bg-red-900/20 border-b border-red-100 dark:border-red-800 flex-shrink-0">
                <AlertCircle className="w-3.5 h-3.5 text-red-500 dark:text-red-400 flex-shrink-0 mt-0.5" />
                <p className="text-xs text-red-600 dark:text-red-300 flex-1">{visError}</p>
                <button onClick={() => setVisError('')} className="p-0.5 rounded hover:bg-red-100 dark:hover:bg-red-900/30" title="Dismiss">
