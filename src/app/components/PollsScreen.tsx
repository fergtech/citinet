...
import { Vote, Plus, X, Check, Lock, RefreshCw, Loader2, ChevronLeft, Clock, Link2, CheckCircle2, XCircle } from 'lucide-react';
...
export function PollsScreen({ onBack }: PollsScreenProps) {
  ...
  // Load open requests when create modal opens (for mod linking)
  useEffect(() => {
  ...
  async function handleVote(poll: Poll, optionIndex: number) {
    ...
    try {
      ...
      // Reload to pick up any auto-close
      load();
    } catch {
      setPolls(prev);
    } finally {
                ...
                <div className="space-y-4">
                  {activePoll.map(poll => (
                    <PollCard key={poll.id} poll={poll} isMod={isMod}
                      ...
                      onVote={idx => handleVote(poll, idx)} onClose={() => handleClose(poll.id)} />
                  ))}
                </div>
              </section>
            )}
                ...
                <div className="space-y-4">
                  {closedPolls.map(poll => (
                    <PollCard key={poll.id} poll={poll} isMod={false}
                      voting={false} closing={false} onVote={() => {}} onClose={() => {}} />
                  ))}
                </div>
              </section>
            )}
...
function PollCard({ poll, isMod, voting, closing, onVote, onClose }: PollCardProps) {
    ...
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 overflow-hidden">
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-white leading-snug flex-1">{poll.question}</h3>
          <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
            {isClosed ? (
              poll.passed === true  ? (
                <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300">
                ...
                <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400">
            ...
            {isMod && !isClosed && (
              <button onClick={onClose} disabled={closing} className="text-[10px] font-medium text-slate-400 hover:text-red-500 transition-colors">
                {closing ? '…' : 'Close'}
              </button>
            )}
          </div>
        ...
        {poll.request_problem && (
          <div className="flex items-center gap-1.5 mb-3 text-[10px] text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-2.5 py-1 rounded-lg w-fit max-w-full">
        ...
        <div className="space-y-2">
          {poll.options.map((opt, i) => {
            ...
            return (
              <button key={i} onClick={() => !isClosed && !voting && onVote(i)}
                disabled={isClosed || voting}
                className={`w-full relative text-left rounded-xl overflow-hidden transition-all ${isClosed || hasVoted ? 'cursor-default' : 'hover:shadow-sm cursor-pointer'} ${isMyVote ? 'ring-2 ring-indigo-500/40' : ''}`}
              >
                {showBars && (
                  <div
                    className={`absolute inset-0 transition-all duration-500 ${
                      isWinner ? 'bg-indigo-100 dark:bg-indigo-900/30'
                      : isMyVote ? 'bg-blue-50 dark:bg-blue-900/20'
                      : 'bg-slate-50 dark:bg-zinc-800'
                    }`}
                    style={{ width: `${pct}%` }}
                  />
                )}
                <div className={`relative flex items-center justify-between px-3.5 py-2.5 rounded-xl border ${
                  isMyVote ? 'border-indigo-400/60 dark:border-indigo-500/40' : 'border-slate-200 dark:border-zinc-700'
                } ${!showBars ? 'bg-slate-50 dark:bg-zinc-800 hover:bg-slate-100 dark:hover:bg-zinc-700' : ''}`}
                >
                  <span className={`text-sm ${isMyVote ? 'font-semibold text-indigo-700 dark:text-indigo-300' : 'text-slate-700 dark:text-slate-300'}`}>
                    {opt}
                  </span>
                  <div className="flex items-center gap-2 shrink-0">
                    {showBars && (
                      <span className={`text-xs font-medium ${atThreshold ? 'text-emerald-600 dark:text-emerald-400 font-bold' : isMyVote ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-zinc-500'}`}>
                        {pct}%
                      </span>
                    )}
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center justify-between text-xs text-slate-400 dark:text-zinc-500">
            <span>{totalVotes} vote{totalVotes !== 1 ? 's' : ''}{poll.member_count > 0 ? ` · ${quorumPct}% of members` : ''}</span>
            {poll.created_by_username && <span>by {poll.created_by_username}</span>}
          </div>
          ...
          {poll.quorum_pct > 0 && (
            <div>
              <div className="flex items-center justify-between text-[10px] text-slate-400 dark:text-zinc-500 mb-1">
                <span>Quorum {quorumMet ? <span className="text-emerald-500 font-semibold">✓ met</span> : `needed: ${poll.quorum_pct}%`}</span>
                <span>{Math.ceil(poll.member_count * poll.quorum_pct / 100)} votes required</span>
              </div>
              <div className="h-1.5 bg-slate-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${quorumMet ? 'bg-emerald-500' : 'bg-indigo-400'}`}
          ...
          {/* Pass threshold note (when open and not yet voted) */}
          {!isClosed && !hasVoted && poll.pass_pct !== 50 && (
            <p className="text-[10px] text-slate-300 dark:text-zinc-600">
