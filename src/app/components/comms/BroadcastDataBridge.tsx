...
// that's conditionally unmounted on minimize but comments/hearts/join-
// requests must keep accumulating in the background. Renders as a direct,
// unconditional child of <LiveKitRoom>.
  ...
  useEffect(() => {
    function handleData(payload: Uint8Array) {
      ...
      if (message.type === 'comment') {
        ...
        if (message.requesterId !== myIdentity) return; // not addressed to me
        if (message.accepted) {
          approvePublish();
          addComment({
        ...
        // I'm never the one who sent this (the host doesn't receive its own
        // room-wide data back), so this only ever reaches viewers/guests:
        // end their own session too instead of leaving them stranded.
        end();
      }
    }
    ...
    // guests. Hidden participants (permissions.hidden) are the "Live now"
    // card's silent camera-preview connections — excluded here rather than
    // spamming an announcement every time someone merely has Messages open.
    ...
    // deleteRoom() call force-disconnects everyone still in it at the
    // protocol level, independent of whether the room-wide 'broadcast_ended'
    // data message above happened to reach this client first. Also the
    // backstop for any other reason the connection drops (network loss,
    // server restart).
