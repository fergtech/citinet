 ...
 *   - Passphrase backup: both keys encrypted under an Argon2id-derived key, stored server-side
 *
...
/**
 * Generates a 7-word recovery phrase (~56 bits of entropy) from a fixed
 * 256-word list. One random byte per word -- 256 is a power of two, so
 * `crypto.getRandomValues` indexing is exactly uniform, no modulo bias.
 * This is the recovery secret going forward, generated by the app rather
 * than chosen by the user, specifically so it can't be a guessable password.
 */
export async function generateRecoveryPhrase(): Promise<string> {
  const { RECOVERY_WORDS } = await import('./recoveryWords');
...
// Argon2id, not PBKDF2 -- OWASP's current top pick specifically because it's
// memory-hard, which resists GPU/ASIC-accelerated offline cracking far better
// than a pure-iteration KDF. Params follow OWASP's minimum baseline for
// Argon2id (m=19MiB, t=2, p=1); WebCrypto has no native Argon2 support, so
// this runs via WASM (hash-wasm) instead of crypto.subtle.
const ARGON2ID_PARAMS = { memorySize: 19_456, iterations: 2, parallelism: 1, hashLength: 32 } as const;
...
async function deriveWrappingKey(passphrase: string, salt: Uint8Array<ArrayBuffer>): Promise<CryptoKey> {
  ...
  // hash-wasm returns Uint8Array<ArrayBufferLike>; importKey needs a concrete ArrayBuffer.
  const keyBytes = new Uint8Array(rawKeyBytes.length);
  keyBytes.set(rawKeyBytes);
  return crypto.subtle.importKey('raw', keyBytes, { name: 'AES-GCM' }, false, ['encrypt', 'decrypt']);
}

/**
 * Pre-2026-08-05 backups were wrapped with PBKDF2, not Argon2id -- the
 * KeyBackupPayload format has no version marker, so a backup predating the
 * switch is otherwise silently unreadable by deriveWrappingKey even with the
 * exact right passphrase (wrong algorithm, not wrong passphrase). Used only
 * as a decrypt-side fallback; every new backup is written under Argon2id.
 */
async function deriveWrappingKeyLegacyPbkdf2(passphrase: string, salt: Uint8Array<ArrayBuffer>): Promise<CryptoKey> {
  const mat = await crypto.subtle.importKey('raw', encode(passphrase), 'PBKDF2', false, ['deriveKey']);
  return crypto.subtle.deriveKey(
...
export async function decryptKeyBackup(
  ...
  const ct   = b64dec(backup.encrypted_payload);
  ...
  try {
    ...
    // Not an Argon2id-wrapped backup -- try the pre-2026-08-05 PBKDF2 format
    // before giving up, so backups created before that switch still restore.
  }
  try {
    const wk = await deriveWrappingKeyLegacyPbkdf2(passphrase, salt);
    const plain = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, wk, ct);
    return JSON.parse(new TextDecoder().decode(plain)) as BackedUpKeys;
  } catch {
    return null;
