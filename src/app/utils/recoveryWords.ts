/**
 * Fixed 256-word list for recovery-phrase generation (see generateRecoveryPhrase
 * in crypto.ts). Exactly 256 entries so each word contributes exactly 8 bits of
 * entropy -- 7 words = 56 bits, well beyond what's practical to brute-force
 * against an Argon2id-wrapped backup. All short, common, unambiguous English
 * words with no near-duplicates (no plural/singular pairs, no homophones) so a
 * word is easy to recognize even if slightly mistyped.
 */
