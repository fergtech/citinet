...
function getCaddyfileContent(config: HubScriptConfig): string {
  return [
    ...
    '    # fetching a freshly-issued cert, without Docker-socket access. Caddy',
    '    # checks the request\'s Origin header against this list regardless of',
    '    # enforce_origin -- citinet-api reaches this over the Docker network',
    '    # by service name, so that\'s the origin it has to be allowed to send.',
    '    admin 0.0.0.0:2019 {',
...
function buildBashTailscaleSection(config: HubScriptConfig): string[] {
  ...
  return [
    ...
    '',
    '# Capture the public URL from Tailscale status (available once authenticated)',
    ...
    '# Run in background so the script continues (funnel keeps running after script exits)',
    ...
    'fi',
    'sleep 3  # give funnel a moment to initialize',
...
function generateBashScript(config: HubScriptConfig): string {
  ...
  const lines = [
    ...
    '  ok "Existing configuration found -- preserving secrets and data"',
    'else',
    ...
    '  ok "Restarting API container with public tunnel URL..."',
    '  $COMPOSE_CMD up -d --no-deps --force-recreate citinet-api 2>/dev/null || true',
    'fi',
...
function generatePowerShellScript(config: HubScriptConfig): string {
  ...
  const lines = [
    ...
    ...(isTailscale ? [
      ...
      'if ($TunnelUrl) {',
      '  Ok "Restarting API container with public tunnel URL..."',
      '  Invoke-Expression "$ComposeCmd up -d --no-deps --force-recreate citinet-api" 2>$null | Out-Null',
      '}',
    ] : []),
    'Ok "Services starting..."',
    '',
